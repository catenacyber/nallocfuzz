...
static int backtrace_callback_print (void *vdata, uintptr_t pc,
...
static void fuzz_nalloc_print_backtrace(const char *op, size_t size) {
    struct backtrace_data data;

    data.index = 0;
    data.max = 64;
    fprintf(stderr, "NULL alloc in %d run: %s(%zu) \n", fuzz_nalloc_runs, op, size);
    backtrace_full (backtrace_state, 0, backtrace_callback_print, backtrace_error_donothing, &data);
}
...
static bool fuzz_nalloc_fail(size_t size, const char *op) {
    ...
    if (((fuzz_nalloc_random_state ^ fuzz_nalloc_magic) & fuzz_nalloc_bitmask) == 0 && fuzz_nalloc_runs != 1) {
        ...
        if (fuzz_nalloc_verbose) {
            fuzz_nalloc_print_backtrace(op, size);
        }
        __sync_fetch_and_sub(&fuzz_nalloc_running, 1);
        return true;
    }
...
void fuzz_nalloc_init(const char * prog) {
    ...
    if (verbose || getenv("FUZZ_REPRODUCE_VERBOSE")) {
...
int __attribute__((weak)) NaloFuzzerInitialize(int *argc, char ***argv);

int LLVMFuzzerInitialize(int *argc, char ***argv) {
    fuzz_nalloc_init((*argv)[0]);
    if (NaloFuzzerInitialize) {
        return NaloFuzzerInitialize(argc, argv);
    }
    return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    int r = NaloFuzzerTestOneInput(data, size);
    __sync_fetch_and_sub(&fuzz_nalloc_running, 1);
    return r;
}
